...
def capture_image(side):
    ...
    if cam_cfg["debug_cam"]:
        if LOGGING_TOGGLE: logger.warning(f"Camera Properties: {cap.get(cv2.CAP_PROP_FRAME_WIDTH)}x{cap.get(cv2.CAP_PROP_FRAME_HEIGHT)} at {cap.get(cv2.CAP_PROP_FPS)} FPS")
    ...
    if ret:
        ...
        if LOGGING_TOGGLE: logger.warning("Camera capture successful for %s side", side)
        file_path = os.path.join(cam_cfg["save_path"], f"{side}_image.jpg")
        cv2.imwrite(file_path, frame)
        if LOGGING_TOGGLE: logger.warning("Saved %s image to %s", side, file_path)

    if not ret:
            if LOGGING_TOGGLE: logger.error("Camera capture failed for %s side", side)
            return None
