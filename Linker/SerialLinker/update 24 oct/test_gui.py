...
class Backend:
    ...
    # --- Simulation toggles ---                                                                            
    def sim_toggle_board(self): self._sim_board_present = not self._sim_board_present                   # Backend_Function_9
    
    def sim_press_start(self): self._sim_start_pressed = True                                           # Backend_Function_10        
    
    def sim_toggle_enable(self): self._sim_toggle_on = not self._sim_toggle_on                          # Backend_Function_11
    
    def sim_toggle_curtain(self): self._sim_light_curtain_clear = not self._sim_light_curtain_clear     # Backend_Function_12
...
class SerialLinkerApp(tk.Tk):
    ...
    def verify_login(self,username, password):
        ...
        if username == "srajan" or username == "84J3F8MO3I771U3J":
    ...
    def log_to_csv(self, operator, board, left_sn, right_sn, result, msg, left_img=None, right_img=None):
        ...
        base_img_dir = os.path.expanduser("~/log_serial_linker")
        base_dir = os.path.expanduser("/mt/barcode_dropbox")
        log_dir = os.path.join(base_dir, "logs")
        failed_dir = os.path.join(base_img_dir, "failed_links")
        ...
        if not result and left_img and right_img:
            try:
                ...
                print(f"[DEBUG_INFO] Saved failed pair images: {left_fail}, {right_fail}")
            except Exception as e:
